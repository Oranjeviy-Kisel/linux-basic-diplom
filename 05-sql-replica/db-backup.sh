...
if [[ `mysql -uroot -p123 --skip-column-names -e "SHOW DATABASES;" | cat - | grep mysql` -eq "mysql" ]]; 
  then 
    echo "Database with name mysql found, creating backup"
    db="mysql"
    for t in `mysql -uroot -p123 -e "SHOW TABLES from $db" | cat -`;
      do
        mysqldump -uroot -p123 $db $t --add-drop-table --events --routines --master-data=2 > /home/backup-$dt/$t.sql
      done
